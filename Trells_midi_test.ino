...
void loop() {
  ...
  while (trellis.available()){
    ...
    if (e.bit.EVENT == KEY_JUST_PRESSED) {
      ...
      trellis.noteOn(FIRST_MIDI_NOTE+key, 120);
    }
    else if (e.bit.EVENT == KEY_JUST_RELEASED) {
      Serial.println(" released\n");
      trellis.setPixelColor(key, 0x0);
      trellis.noteOff(FIRST_MIDI_NOTE+key, 120);
    }
  }
  ...
  if (abs(event.acceleration.y) < 2.0) {  // 2.0 m/s^2
    ...
    ybend = 0; // 8192 means no bend
  } else {
    if (event.acceleration.y > 0) {
      ybend = ofMap(event.acceleration.y, 2.0, 10.0, 0, 127, true);  // 2 ~ 10 m/s^2 is downward bend
    } else {
      ybend = ofMap(event.acceleration.y, -2.0, -10.0, 0, 127, true);  // -2 ~ -10 m/s^2 is upward bend
    }
  }
  if (ybend != last_ybend) {
    Serial.print("Y pitchbend: "); Serial.println(ybend);
    trellis.controlChange(yCC, ybend);
    last_ybend = ybend;
  }
  ...
  if (abs(event.acceleration.z) < 2.0) {  // 2.0 m/s^2
    // don't make any bend unless they've really started moving it
    zbend = 0; // 8192 means no bend
  } else {
    if (event.acceleration.z > 0) {
      zbend = ofMap(event.acceleration.z, 2.0, 10.0, 0, 127, true);  // 2 ~ 10 m/s^2 is downward bend
    } else {
      zbend = ofMap(event.acceleration.z, -2.0, -10.0, 0, 127, true);  // -2 ~ -10 m/s^2 is upward bend
    }
  }
  if (zbend != last_zbend) {
    Serial.print("z pitchbend: "); Serial.println(zbend);
    trellis.controlChange(zCC, zbend);
    last_zbend = zbend;
  }
