...
int THRESHOLD1 = 50;            //threshold amount
int THRESHOLD2 = 120;
int THRESHOLD3 = 30;
int THRESHOLD4 = 90;
...
void loop(){
  ...
  if(buttonVal == bounceCheck){           //if val is the same then not a bounce
   ...
   if(irVal >= THRESHOLD4){
     if(abs(irVal - previrVal) >= THRESHOLD1) //set 0 as default
         
       midiOUT(0xB0, 13, mappedirVal); //Control Change (Ch 1), Controller 7 - default control for volume.
       previrVal = irVal;
       midicc1State = 1;
     }
   else if((irVal <= THRESHOLD3) && midicc1State == 1){
     midiOUT(0xB0, 13, 127);
     delay(10);
     midiOUT(0xB0, 13, 124);
     midicc1State = 0; 
   ...
   if(irVal >= THRESHOLD2){         // start midi note send// determine ir state, set digital value
     irState = 1;                   // digital value set to ON 
   }
   if(irVal <= THRESHOLD2){  
     irState = 0;                   //digital value is set to OFF
   }
   ...
   if(ir1Val >= THRESHOLD4){
     if(abs(ir1Val - previr1Val) >= THRESHOLD1) //set 0 as default
                 
     midiOUT(0xB0, 14, mappedir1Val); //Control Change (Ch 1), Controller 2 - 
     previr1Val = ir1Val;
     midicc2State = 1;
     }
   else if((ir1Val <= THRESHOLD3) && midicc2State == 1){
     midiOUT(0xB0, 14, 127);
     delay(10);
     midiOUT(0xB0, 14, 124);
     midicc2State = 0; 
   ...
   if(ir1State == 0 && note1State ==1){ 
     midiOUT(0x90, 69, 0); //Note ON (CH 1), middle C, zero velocity turns note off
     note1State = 0;
     }
     
   if(ir1State == 1 && note1State == 0){
     midiOUT(0x90, 69, 127); //Note ON (CH 1), middle C, velocity 127
     note1State = 1;
     }
   //note 3
     //ir 3
   ...
   if(ir3Val >= THRESHOLD4){
     if(abs(ir3Val - previr3Val) >= THRESHOLD1) //set 0 as default
         
     midiOUT(0xB0, 15, mappedir3Val); //Control Change (Ch 1), Controller 7 - default control for volume.
     previr3Val = ir3Val;
     midicc3State = 1;
     }
   else if((ir3Val <= THRESHOLD3) && midicc3State == 1){
     midiOUT(0xB0, 15, 127);
     delay(10);
     midiOUT(0xB0, 15, 124);
     midicc3State = 0; 
   ...
   if(ir3State == 0 && note3State ==1){ 
     midiOUT(0x90, 71, 0); //Note ON (CH 1), middle C, zero velocity turns note off
     note3State = 0;
     }
     
   if(ir3State == 1 && note3State == 0){
     midiOUT(0x90, 71, 127); //Note ON (CH 1), middle C, velocity 127
     note3State = 1;
     }         
    //note 4
     //ir 4
   ...
   if(ir4Val >= THRESHOLD4){
     if(abs(ir4Val - previr4Val) >= THRESHOLD1) //set 0 as default
            
     midiOUT(0xB0, 16, mappedir4Val); //Control Change (Ch 1), Controller 7 - default control for volume.
     previr4Val = ir4Val;
     midicc4State = 1;
     }
   else if((ir4Val <= THRESHOLD3) && midicc4State == 1){
     midiOUT(0xB0, 16, 127);
     delay(10);
     midiOUT(0xB0, 16, 124);
     midicc4State = 0; 
   ...
   if(ir4State == 0 && note4State ==1){ 
     midiOUT(0x90, 72, 0); //Note ON (CH 1), middle C, zero velocity turns note off
     note4State = 0;
     }
     
   if(ir4State == 1 && note4State == 0){
     midiOUT(0x90, 72, 127); //Note ON (CH 1), middle C, velocity 127
     note4State = 1;
     }   
}      
