...
void handleMessage(AdafruitIO_Data *data) {
  ...
  if (stringTwo == "Cloudy"){
    ...
    for(int i=0; i<NUMPIXELS; i++){
      pixels.setPixelColor(i, pixels.Color(180, 0, 255));
      pixels.show();
    }
  }
   ...
   ////////////////////Twitter control///////////
  if (stringTwo == "makerlabgreen"){
    Serial.println("Twitter green!");
    for(int i=0; i<NUMPIXELS; i++){
      pixels.setPixelColor(i, pixels.Color(0, 255, 0));
      pixels.show();
    }
  }
  if (stringTwo == "makerlabblue"){
    Serial.println("twitter blue");
    for(int i=0; i<NUMPIXELS; i++){
      pixels.setPixelColor(i, pixels.Color(0, 0, 255));
      pixels.show();
    }
  }
  if (stringTwo == "makerlabpurple"){
    Serial.println("twitter purple");
    for(int i=0; i<NUMPIXELS; i++){
      pixels.setPixelColor(i, pixels.Color(180, 0, 255));
      pixels.show();
    }
  }
   if (stringTwo == "makerlabred"){
    Serial.println("twitter red");
    for(int i=0; i<NUMPIXELS; i++){
