...
int THRESHOLD1 = 15;            //threshold amount
int THRESHOLD2 = 15;
...
void loop(){
  ...
  if(buttonVal == bounceCheck){           //if val is the same then not a bounce
   ...
   mappedirVal = map(irVal, 0, 600, 0, 127);  //map value to 0-127
 
   if(abs(mappedirVal - previrVal) >= THRESHOLD1){ //set 0 as default
     if (abs(irVal) == 0 ){
       mappedirVal = 0;    
     }     
     midiOUT(0xB0, 92, mappedirVal); //Control Change (Ch 1), Controller 7 - default control for volume.
   ...
   mappedir1Val = map(ir1Val, 0, 600, 0, 127);  //map value to 0-127   //it seems that min midi cc effects gitter in two bad servos
 
   if(abs(mappedir1Val - previr1Val) >= THRESHOLD1){ //set 0 as default
     if (abs(ir1Val) == 0 ){
       mappedir1Val = 0;    
     }     
     midiOUT(0xB0, 91, mappedir1Val); //Control Change (Ch 1), Controller 2 - 
   ...
   mappedir3Val = map(ir3Val, 0, 600, 0, 127);  //map value to 0-127
 
   if(abs(mappedir3Val - previr3Val) >= THRESHOLD1){ //set 0 as default
     if (abs(ir3Val) == 0 ){
       mappedir3Val = 0;    
     }     
     midiOUT(0xB0, 98, mappedir3Val); //Control Change (Ch 1), Controller 7 - default control for volume.
   ...
   mappedir4Val = map(ir4Val, 0, 600, 0, 127);  //map value to 0-127
 
   if(abs(mappedir4Val - previr4Val) >= THRESHOLD1){ //set 0 as default
     if (abs(ir4Val) == 0 ){
       mappedir4Val = 0;    
     }     
     midiOUT(0xB0, 95, mappedir4Val); //Control Change (Ch 1), Controller 7 - default control for volume.
