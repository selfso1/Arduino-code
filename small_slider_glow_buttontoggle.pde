...
void loop() {
  ...
  if(buttonVal == bounceCheck){           //if val is the same then not a bounce
    ...
    if (buttonVal == HIGH && noteState == 0) {   //check if the input is HIGH
        buttonState = 1;
    }
    if (buttonVal == HIGH && noteState == 1)  {
        buttonState = 0;  
    }
    ...
    if (buttonVal == LOW && buttonState == 0)  {
      ...
      if (buttonState == 1 && noteState == 0 && toggleState == 1){
        midiOUT(0x90, 60, 127); //note on
        noteState = 1;
        digitalWrite(ledPin, HIGH);
    }
    
      if (buttonState == 0 && noteState == 1 && toggleState == 0){
